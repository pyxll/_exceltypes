...
*****************************************************************
This module is taken from the win32com demos.

It is modified to use the PyIRTDUpdateEvent class provided by
the exceltypes package as the makepy IDispatch version of the
same class no longer works since Excel 2007.

Modifications from the original are marked with the comment
...
# EXCELTYPES_MODIFICATION
import exceltypes # exceltypes is needed for the IPyIRTDUpdateEvent class.
...
class ExcelRTDServer(object):
  ...
  def ServerStart(self, CallbackObject):
    ...
    self.__callback = CallbackObject.QueryInterface(exceltypes.IID_IRTDUpdateEvent)
